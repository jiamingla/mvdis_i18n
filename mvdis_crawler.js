...
module.exports = {
  /* 爬出該地點該時間可以報考的名額 */
  locations_query: async function (licenseTypeCode, expectExamDateStr, dmvNo) {
    try {
      ...
      if (
        headerMessage.text() === "該條件查詢不到符合的考試場次，敬請重新選擇"
      ) {
      ...
      for (let i = 0; i < search_table.length / 4; i++) {
        // 走訪
        const t = i * 4;
        const date = search_table.eq(t).text().replace(/\t/g, "").trim();
  ...
  sign_up: async function (
    ...
    try {
      ...
      console.log(/報名成功/.test($("script").text()));
      return true;
    } catch (error) {
      console.error(error);
      return false;
